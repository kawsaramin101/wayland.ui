...
static void scroll_to_selected(widget_t *w) {
    list_data_t *d = w->data;
...
static void list_on_mouse_move(widget_t *w, wl_mouse_move_event_t *e) {
    ...
    /* check if mouse is inside list bounds */
    ...
    int rel_y = (int)e->y - w->y + d->scroll_y;
    int row   = rel_y / d->item_height;
    d->hovered_row = (row >= 0 && row < w->child_count) ? row : -1;
}
...
static void list_on_key(widget_t *w, wl_key_event_t *e) {
    ...
    switch (e->sym) {
        ...
        case 0xFF54: /* down */
            ...
            else d->selected = w->child_count - 1;
            break;
        case 0xFF55: /* page up */
            d->selected -= vis;
